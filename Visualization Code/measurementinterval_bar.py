#####measurementinterval_bar.py#####

######################################################
#Bar chart graphic that  shows measurement intervals #
######################################################

#Variables
#line 16 : correct directory to local streamflow catalog address
...
width = 0.3
bar_colors = ['darkorange', 'darkgoldenrod', 'gold', 'khaki','lightyellow','yellow','orange','red']
fig, ax = plt.subplots()
rects1 = ax.bar(x - width/8,counts,label='labels',color=bar_colors)
ax.yaxis.set_major_formatter(mpl.ticker.StrMethodFormatter('{x:,.0f}')) #formats integer values to #,###
